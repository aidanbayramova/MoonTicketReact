...
const toCard = (product) => {
    ...
    return {
        id: product.id,
        title: product.name,
        genre: genreParts.length ? genreParts.join(" • ") : "Tourism",
        ...
        fetchProducts()
            .then((list) => {
                ...
                const mapped = (filtered.length ? filtered : list)
                    ...
                    .map(toCard)
                    .filter((c) => c.img);
                if (mapped.length) setTours(mapped);
            })
            .catch(() => {});
        return () => {
            active = false;
                                ...
                                <div className="tourism-card-inner">
                                    <div className="tourism-card-image-container">
                                        ...
                                        <div className="tourism-card-actions">
                                            <button
                                                className="tourism-info-btn"
                                                onClick={() => tour.id && (window.location.href = `/event/tourismdetail/${tour.id}`)}
                                            >
                                                <Info size={20} /> Info
                                            </button>
